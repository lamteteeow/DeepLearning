...
class Conv(BaseLayer):  # inherited member "trainable"
    def __init__(self, stride_shape, convolution_shape, num_kernels):
        ...
        self.convolution_shape = (
            convolution_shape  # Shape either 1-D -> [c,m]; m = kernel length or 2-D -> [c,m,n]
        )
    ...
    def _forward_1d(self, input_tensor):
        ...
        padded_input = np.pad(
            input_tensor, ((0, 0), (0, 0), (padding_length, padding_length)), mode="constant"
        )

        # Add padding to output tensor
        output_length = (input_length + 2 * padding_length - kernel_length) // self.stride_shape[0][
            0
        ] + 1
        ...
        for b in range(batch_size):
            for k in range(self.num_kernels):
                for i in range(output_length):
                    ...
                    output_tensor[b, k, i] = (
                        np.sum(padded_input[b, :, start:end] * self.weights[k, :, :]) + self.bias[k]
                    )
    ...
    def _forward_2d(self, input_tensor):
        ...
        padded_input = np.pad(
            ...
            (
                (0, 0),
                (0, 0),
                (padding_height_input, padding_height_output),
                (padding_width_input, padding_width_output),
            ),
            mode="constant",
            constant_values=0,
        )
        ...
        output_height = (
            input_height + padding_height_input + padding_height_output - kernel_height
        ) // self.stride_shape[0] + 1
        output_width = (
            input_width + padding_width_input + padding_width_output - kernel_width
            ...
            for k in range(self.num_kernels):
                ...
                for c in range(input_channels):
                    correlation_result = correlate(
                        padded_input[b, c], self.weights[k, c], mode="valid"
                    )
                    # Ensure the dimensions are correct by applying stride
                    sliced_result = correlation_result[
    ...
    def _backward_2d(self, error_tensor):
        ...
        padded_input = np.pad(
            ...
            (
                (0, 0),
                (0, 0),
                (self.padding_height_before, self.padding_height_after),
                (self.padding_width_before, self.padding_width_after),
            ),
            mode="constant",
            constant_values=0,
        )
