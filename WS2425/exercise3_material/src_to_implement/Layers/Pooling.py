...
class Pooling(BaseLayer):
    ...
    def forward(self, input_tensor):
        ...
        self.mask = np.zeros((input_tensor.shape[0], input_tensor.shape[1], output_shape[2], output_shape[3], self.pooling_shape[0], self.pooling_shape[1]))
    ...
    def backward(self, error_tensor):
        ...
        for i in range(error_tensor.shape[2]):
            for j in range(error_tensor.shape[3]):
                ...
                input_grad[:, :, 
                           i*self.stride_shape[0]:i*self.stride_shape[0]+self.pooling_shape[0], 
                           j*self.stride_shape[1]:j*self.stride_shape[1]+self.pooling_shape[1]] += grad * self.mask[:, :, i, j] 
        return input_grad
