...
class TestCheckers(unittest.TestCase):
    ...
    def testPattern(self):
        ...
        np.testing.assert_almost_equal(
            c.output,
            self.reference_img,
            err_msg="Check if your pattern starts with a" "black tile in the upper left corner.",
        )
...
class TestGen(unittest.TestCase):
    ...
    def testInit(self):
        ...
        np.testing.assert_almost_equal(
            gen.next()[0],
            gen2.next()[0],
            err_msg="Possible error: Maybe the data is already " "shuffled during initialization.",
        )
        np.testing.assert_almost_equal(
            gen.next()[1],
            gen2.next()[1],
    ...
    def testDuplicate(self):
        ...
        self.assertFalse(
            np.any(np.all(sample == b1_without_sample, axis=(1, 2, 3))),
        ...
        self.assertFalse(
            np.any(np.all(sample == b2[0], axis=(1, 2, 3))),
    ...
    def testEpoch(self):
        ...
        self.assertEqual(
            0,
            gen.current_epoch(),
            msg="Possible error: The counter for the current epoch does not work."
    ...
    def testMirroring(self):
        ...
        np.testing.assert_almost_equal(
            np.sum(vertical * horizontal * horizontal_vertical),
            0,
    ...
    def testLabels(self):
        ...
        self.assertTrue(
            np.issubdtype(np.array(label).dtype, np.integer),
            msg="Possible error: The generator does not return integer labels. Please make sure to cast "
            "your labels to integers.",
        )
        ...
        for i, (k, (outcome, p)) in enumerate(bonus_points.items()):
            table.append(
                [
                    ...
                    k,
                    outcome,
                    "0 / {} (%)".format(p) if outcome == "FAIL" else "{} / {} (%)".format(p, p),
